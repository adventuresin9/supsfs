...
/* by adventuresin9                                */
/*                                                 */
/* Modification of the gpsfs program included with */
/* 9 Front to output information from the GLONASS  */
/* positioning system ☭                            */
/*                                                 */
/* While the compiled command names are different  */
/* all other accociated names and formatting are   */
/* kept the same to work with timesync             */
/*                                                 */
/* Tested with a PA1616D reciever hooked           */
/* up to a Raspberry Pi 3B                         */
/***************************************************/
...
readstats(Req *r)
{
	...
	p = seprint(p, buf + sizeof buf, "%lud checksum errors, comrade!\n", checksumerrors);
...
tryagain:
	for(;;){
		...
		while(getonechar(t) != '\n' && n < 4096)
			n++;
		if(n == 4096)
			sysfatal("preposterous ☭ps line, comrade! wrong baud rate?");
		fprint(2, "ridiculous ☭ps line: %d bytes\n", n);
	}
	*p = 0;
	for(p = s; isdigit(*p); p++)
		;
